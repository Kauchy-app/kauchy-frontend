    ...
    if (!user) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh]">
                ...
                <h2 className="text-xl font-bold text-zinc-900 dark:text-white">Sign in to view notifications</h2>
            ...
            <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-sm border border-zinc-100 dark:border-zinc-800 overflow-hidden">
                {notifications.length === 0 ? (
                    <div className="py-20 flex flex-col items-center justify-center text-center">
                        ...
                        <p className="text-zinc-500 dark:text-zinc-400">No new notifications right now.</p>
                    </div>
                ) : (
                    <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
                        {notifications.map((notif) => (
                            <div
                                key={notif.id}
                                className={`group relative p-5 flex items-start gap-4 cursor-pointer transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800/50 ${
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0 pr-8">
                                    <h4 className={`text-sm font-bold mb-1 ${notif.is_read ? 'text-zinc-700 dark:text-zinc-300' : 'text-zinc-900 dark:text-white'}`}>
                                        {notif.title}
                                    </h4>
                                    <p className={`text-sm mb-2 leading-relaxed ${notif.is_read ? 'text-zinc-500 dark:text-zinc-400' : 'text-zinc-700 dark:text-zinc-200'}`}>
                                        {notif.message}
                                    </p>
                                ...
                                <button
                                    ...
                                    className="absolute right-4 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 p-2 rounded-full hover:bg-zinc-200 dark:hover:bg-zinc-700 text-zinc-400 hover:text-red-500 transition-all focus:opacity-100"
